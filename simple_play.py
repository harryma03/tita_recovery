from configs.wheeled_titatit_constraint_him import (
    WheeledTitatitConstraintHimRoughCfg,
    WheeledTitatitConstraintHimRoughCfgPPO,
    WheeledTitatitRecoveryCfg,
    WheeledTitatitRecoveryCfgPPO,
)
import cv2
import os
...
def play(args):
    ...
    model_dict = torch.load(load_path, map_location=env.device)
    ...
    for i in range(num_frames):
        ...
        if getattr(getattr(env.cfg, 'recovery', None), 'enabled', False):
            env.commands[:] = 0
        else:
            env.commands[:,0] = 1
            env.commands[:,1] = 0
...
if __name__ == '__main__':
    ...
    task_registry.register("wheeled_titatit_recovery",LeggedRobot,WheeledTitatitRecoveryCfg(),WheeledTitatitRecoveryCfgPPO())
