...
class LeggedRobot(BaseTask):
    ...
    def check_termination(self):
        ...
        """
        if getattr(getattr(self.cfg, "recovery", None), "enabled", False):
    ...
    def _quat_from_euler_xyz(self, roll, pitch, yaw):
        ...
        quat = torch.zeros(roll.shape[0], 4, device=self.device)
    ...
    def _reset_recovery_root_states(self, env_ids):
        ...
        height_range = self.cfg.recovery.init_base_height_range
        roll = torch_rand_float(-np.pi, np.pi, (num_envs, 1), device=self.device).squeeze(1)
        pitch = torch_rand_float(-np.pi, np.pi, (num_envs, 1), device=self.device).squeeze(1)
        yaw = torch_rand_float(-np.pi, np.pi, (num_envs, 1), device=self.device).squeeze(1)

        mode = torch.randint(0, 4, (num_envs,), device=self.device)
        side_roll = torch.where(
            torch.rand(num_envs, device=self.device) > 0.5,
            torch.ones(num_envs, device=self.device),
            -torch.ones(num_envs, device=self.device),
        ) * (0.5 * np.pi + torch_rand_float(-0.35, 0.35, (num_envs, 1), device=self.device).squeeze(1))
        roll = torch.where(mode == 0, side_roll, roll)
        pitch = torch.where(mode == 1, np.pi + torch_rand_float(-0.35, 0.35, (num_envs, 1), device=self.device).squeeze(1), pitch)
        pitch = torch.where(mode == 2, torch_rand_float(-0.25, 0.25, (num_envs, 1), device=self.device).squeeze(1), pitch)

        self.root_states[env_ids, 2] = self.env_origins[env_ids, 2] + torch_rand_float(height_range[0], height_range[1], (num_envs, 1), device=self.device).squeeze(1)
        self.root_states[env_ids, 3:7] = self._quat_from_euler_xyz(roll, pitch, yaw)
        self.root_states[env_ids, 7:10] = torch_rand_float(-0.3, 0.3, (num_envs, 3), device=self.device)
        self.root_states[env_ids, 10:13] = torch_rand_float(-1.5, 1.5, (num_envs, 3), device=self.device)
    ...
    def _reset_dofs(self, env_ids):
        ...
        """
        if getattr(getattr(self.cfg, "recovery", None), "enabled", False):
            joint_range = getattr(self.cfg.recovery, "init_dof_pos_range", 0.7)
            self.dof_pos[env_ids] = self.default_dof_pos + torch_rand_float(-joint_range, joint_range, (len(env_ids), self.num_dof), device=self.device)
    ...
    def _terrain_heights_for_climbing(self):
        local_x = self.height_points[0, :, 0]
        center_mask = torch.abs(local_x) <= 0.2
        front_mask = local_x >= 0.25

        center_height = torch.mean(self.measured_heights[:, center_mask], dim=1)
        front_height = torch.max(self.measured_heights[:, front_mask], dim=1).values
        obstacle_height = torch.clamp(front_height - center_height, min=0.0)
        obstacle_threshold = getattr(self.cfg.rewards, "base_height_obstacle_threshold", 0.06)
        obstacle_height = torch.where(
            obstacle_height > obstacle_threshold,
            obstacle_height,
            torch.zeros_like(obstacle_height),
        )
        obstacle_max = getattr(self.cfg.rewards, "base_height_obstacle_max", 0.0)
        return center_height, torch.clamp(obstacle_height, max=obstacle_max)

    def _reward_base_height(self):
        # Keep the nominal body height on flat terrain, but lift early when the
        # forward height scan sees a step/high obstacle.
        center_height, obstacle_height = self._terrain_heights_for_climbing()
        obstacle_gain = getattr(self.cfg.rewards, "base_height_obstacle_gain", 0.0)
        target_height = self.cfg.rewards.base_height_target + obstacle_gain * obstacle_height
        base_height = self.root_states[:, 2] - center_height
        return torch.square(base_height - target_height)

    def _reward_recovery_upright(self):
        upright = torch.clamp(-self.projected_gravity[:, 2], min=0.0, max=1.0)
        return torch.square(upright)

    def _reward_recovery_height(self):
        target_height = self.cfg.rewards.base_height_target
        sigma = getattr(self.cfg.recovery, "height_sigma", 0.08)
        height_error = torch.square(self.root_states[:, 2] - target_height)
    ...
    def _reward_recovery_stability(self):
        ...
        lin_vel_error = torch.square(self.base_lin_vel[:, 2])
        return torch.exp(-(ang_vel_error + lin_vel_error) / self.cfg.rewards.tracking_sigma)

    def _reward_recovery_success(self):
        upright = -self.projected_gravity[:, 2] > getattr(self.cfg.recovery, "success_upright_threshold", 0.9)
        high_enough = self.root_states[:, 2] > getattr(self.cfg.recovery, "success_height_threshold", 0.36)
        stable = torch.norm(self.base_ang_vel[:, :2], dim=1) < getattr(self.cfg.recovery, "success_ang_vel_threshold", 0.5)
        return 1.0 * upright * high_enough * stable
    ...
    def _reward_foot_clearance(self):
        center_height, obstacle_height = self._terrain_heights_for_climbing()
        clearance_target = getattr(self.cfg.rewards, "foot_clearance_target", 0.10)
        clearance_gain = getattr(self.cfg.rewards, "foot_clearance_obstacle_gain", 0.0)
        max_clearance = getattr(self.cfg.rewards, "foot_clearance_max_target", clearance_target)
        target_height = clearance_target + clearance_gain * obstacle_height
        target_height = torch.clamp(target_height, max=max_clearance)

        foot_height = self.foot_positions[:, :, 2] - center_height.unsqueeze(1)
        swing_mask = ~self.contact_filt
        moving_mask = (torch.norm(self.commands[:, :2], dim=1) > 0.1).unsqueeze(1)
        clearance_error = torch.square((target_height.unsqueeze(1) - foot_height).clip(min=0.0))
        return torch.sum(clearance_error * swing_mask * moving_mask, dim=1)
   
    def _reward_recovery_default_leg_pos(self):
        leg_ids = [0, 1, 2, 4, 5, 6, 8, 9, 10, 12, 13, 14]
        err = torch.sum(torch.square(self.dof_pos[:, leg_ids] - self.default_dof_pos[:, leg_ids]), dim=1)

        upright = -self.projected_gravity[:, 2] > 0.75
        high_enough = self.root_states[:, 2] > 0.30
        stand_mask = upright * high_enough

        return err * stand_mask
